...
com.atpezms.atpezms.<context>.exception/   # context-specific domain exceptions
```

The `common` package holds shared infrastructure that is not specific to any bounded context:

```
com.atpezms.atpezms.common.exception/   # BaseException hierarchy, GlobalExceptionHandler
com.atpezms.atpezms.common.dto/         # Shared DTOs (e.g. ErrorResponse)
com.atpezms.atpezms.common.config/      # Cross-cutting Spring configuration (e.g. JpaAuditConfig)
com.atpezms.atpezms.common.entity/      # Shared mapped superclasses (e.g. BaseEntity)
com.atpezms.atpezms.common.converter/   # JPA AttributeConverters for cross-cutting persistence concerns (e.g. StringEncryptionConverter)
```
...
- **`AUTO_INCREMENT` is supported natively:** H2 2.x (the version bundled with Spring Boot 3+/4) supports `BIGINT NOT NULL AUTO_INCREMENT` as a built-in alias for `GENERATED BY DEFAULT AS IDENTITY`. No MySQL compatibility mode (`MODE=MySQL`) is required. All migrations use `AUTO_INCREMENT` without any special H2 URL parameters. If you see `AUTO_INCREMENT` in migrations and wonder if MySQL mode is required -- it is not.
...
Established in the Ticketing slice (Phase 1) when Visitor PII fields are implemented.

### 13.1 Mechanism

PII fields (visitor name, email, phone) are encrypted at the application layer using a JPA `AttributeConverter` before being persisted. The rest of the application always sees plain String values; only the database column ever holds ciphertext.

Implemented as: `com.atpezms.atpezms.common.converter.StringEncryptionConverter`

### 13.2 Algorithm

AES-256-GCM (Galois/Counter Mode):
- **Confidentiality:** AES in counter mode produces a ciphertext indistinguishable from random bytes.
- **Integrity:** GCM appends a 128-bit authentication tag. Decryption fails loudly if the ciphertext or tag is altered. This prevents silent data corruption from database-level tampering.
- **No padding required:** GCM is a stream-cipher mode; output length equals input length plus the 16-byte tag.
...
- Base64 adds ~33% overhead. VARCHAR(512) comfortably accommodates any realistic PII value.

### 13.4 Key Management

- Key: 256-bit (32 bytes), provided via the `atpezms.encryption.key` application property as a **Base64-encoded** string (e.g. produced by `openssl rand -base64 32`). The converter Base64-decodes the value at startup and validates that the result is exactly 32 bytes. Setting the property to a raw ASCII string (not Base64-encoded) will produce a startup failure with an `IllegalArgumentException`.
- The constructor validates key length before constructing the `SecretKeySpec`, so misconfiguration is caught immediately at startup rather than silently at the first encryption attempt.
- In production/staging: inject via environment variable or a secrets manager. Never commit a real key.
- In the `test` profile: a fixed committed key is acceptable because the test database is in-memory, ephemeral, and contains no real PII.
- In the `dev` profile: the same test key is used as a placeholder. Documented in `application-dev.properties`.

### 13.5 Random IV Per Call

A fresh 12-byte IV is generated by `SecureRandom` for every `convertToDatabaseColumn` call. This means encrypting the same plaintext twice yields different ciphertexts, preventing dictionary/rainbow-table attacks against the column values. It also means ciphertext values in the database cannot be directly compared for equality — this is intentional; equality checks on PII are done at the service layer after decryption.

### 13.6 Opt-In Via @Convert

The converter uses `@Converter(autoApply = false)`. Each PII field must explicitly declare `@Convert(converter = StringEncryptionConverter.class)`. This prevents accidental encryption of non-PII String fields (e.g. `code`, `name`, `status`).

### 13.7 Spring DI Integration

`StringEncryptionConverter` is a `@Component`. Spring Boot configures Hibernate with `hibernate.resource.beans.container = SpringBeanContainer`, which causes Hibernate to resolve `AttributeConverter` instances from the Spring `ApplicationContext`. This enables `@Value` injection of the encryption key into the converter constructor.

### 13.8 Rules For All Slices

- Only fields explicitly classified as PII (see each slice's design doc) use `@Convert(converter = StringEncryptionConverter.class)`.
- **Never log PII fields** (plaintext or ciphertext). Log the visitor ID or a non-sensitive correlation key instead.
- **Never compare encrypted columns in JPQL/SQL** for equality. Decrypt first via the entity, then compare in Java.

---

## 14. Conventions Not Yet Established
